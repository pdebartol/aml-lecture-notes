...
\usepackage{amsmath,amsfonts,graphicx,dsfont}
...
\newcommand{\lecture}[4]{
   ...
   {\bf Disclaimer}: {\it These notes are adapted from ETH's Advanced Machine Learning Course and Statistical Machine Learning Notes 3, Justin Domke, UMASS.}
   \vspace*{4mm}
}
%
...
\end{equation*}
We have the training error $\hat{R}(f, Z^{train}) = \frac{1}{n}\sum\limits_{i = 1}^{n}L(f^*(x_i), y_i)$ and its minimizer $f^* = \argmin_{f \in C} \hat{R}(f)$.\\
The test error amounts to $\hat{R}(f, Z^{test}) = \frac{1}{m}\sum\limits_{i = n+1}^{m+n}L(f^*(x_i), y_i)$.\\
...
\end{equation*}
What is the probability $P(|\hat{R}(f, Z^{test})-\underset{x}{\mathbb{E}}[R(f^*, X)]|> \epsilon)$?\\
